...
function connectionvpn() {

# Demander à l'utilisateur les informations de connexion
read -p "Adresse du serveur OpenVPN : " SERVER_ADDRESS
read -p "Port OpenVPN (par défaut 1194) : " SERVER_PORT
...
# Créer le fichier de configuration avec les informations fournies
cat <<EOL > "$CONFIG_FILE"
client
dev tun
proto udp
...
# Créer un fichier d'authentification avec le nom d'utilisateur et le mot de passe
echo -e "$USERNAME\n$PASSWORD" > auth.txt

# Connecter OpenVPN en utilisant le fichier de configuration
sudo openvpn --config "$CONFIG_FILE" --auth-user-pass auth.txt

# Supprimer les fichiers temporaires
rm -f "$CONFIG_FILE" auth.txt
